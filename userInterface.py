...
class NGE:
    def __init__(cls):
        ...
        cls.currentSheet = Sheet(0, cls.currentBook)
    ...
    def create_sheet_display_pixels(self):
        ...
        for character in self.NGE.currentSheet.char_list:
            ...
            while(row <= 7):
                ...
                self.sheet_display.create_rectangle(pixel_start_x, pixel_start_y, pixel_start_x+3, pixel_start_y + 3, fill = fill_color, width = 1, outline = fill_color, tags = str(character.obj_num) + " pixels")
                pixel_start_x += 4
                column += 1
                if(column == 8):
                    pixel_start_x = int(coords[0]+1)
